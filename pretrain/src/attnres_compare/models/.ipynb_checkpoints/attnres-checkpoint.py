...
from attnres_compare.config import ModelConfig
from attnres_compare.models.common import (
    CausalSelfAttention,
    ForwardOutput,
    MLP,
    RMSNorm,
...
class DepthMixer(nn.Module):
    ...
    def forward(
        self, sources: list[torch.Tensor], return_stats: bool = False
    ) -> tuple[torch.Tensor, dict[str, Any] | None]:
        if not sources:
            raise ValueError("DepthMixer requires at least one source tensor")
        if len(sources) == 1:
            h = sources[0]
            if not return_stats:
                return h, None
        ...
        weights = torch.softmax(logits, dim=0, dtype=torch.float32)
        h = (weights.unsqueeze(-1).to(dtype=v.dtype) * v).sum(dim=0)
        if not return_stats:
            return h, None
...
class AttnResBlock(nn.Module):
    def __init__(self, cfg: ModelConfig) -> None:
        ...
        self.attn = CausalSelfAttention(
            cfg.d_model, cfg.n_heads, bias=cfg.attn_bias, dropout=cfg.dropout
        )
        self.pre_mlp_mixer = DepthMixer(cfg.d_model, eps=cfg.norm_eps)
        self.mlp_norm = RMSNorm(cfg.d_model, eps=cfg.norm_eps)
        self.mlp = MLP(cfg.d_model, cfg.d_ff, kind=cfg.mlp_type, bias=cfg.attn_bias)
...
class FullAttnResGPT(nn.Module):
    ...
    def forward(
        ...
        if seqlen > self.cfg.max_seq_len:
            raise ValueError(
                f"Sequence length {seqlen} exceeds model max_seq_len {self.cfg.max_seq_len}"
            )
        pos = torch.arange(seqlen, device=input_ids.device)
        x0 = self.tok_emb(input_ids) + self.pos_emb(pos)[None, :, :]
        sources: list[torch.Tensor] = [x0]
        ...
        for block in self.blocks:
            ...
            attn_out = block.attn(block.attn_norm(h))
            if return_aux and aux is not None:
                aux["attn_out_rms"].append(
                    float(tensor_rms(attn_out).detach().cpu().item())
                )
            sources.append(attn_out)
            ...
            mlp_out = block.mlp(block.mlp_norm(h))
            if return_aux and aux is not None:
                aux["mlp_out_rms"].append(
                    float(tensor_rms(mlp_out).detach().cpu().item())
                )
            sources.append(mlp_out)
...
class BlockAttnResGPT(nn.Module):
    def __init__(self, cfg: ModelConfig) -> None:
        super().__init__()
        self.cfg = cfg
        self.block_size_layers = cfg.attnres_block_size or 2
        if self.block_size_layers <= 0:
    ...
    def _max_sources(self) -> int:
        return 1 + math.ceil(self.cfg.n_layers / self.block_size_layers)
    ...
    def _build_sources(
        ...
        partial_delta: torch.Tensor | None,
    ) -> list[torch.Tensor]:
        if partial_delta is None:
            return list(block_reps)
        return [*block_reps, block_input + partial_delta]
    ...
    def forward(
        ...
        pos = torch.arange(seqlen, device=input_ids.device)
        x0 = self.tok_emb(input_ids) + self.pos_emb(pos)[None, :, :]
        block_reps: list[torch.Tensor] = [x0]
        block_input = x0
        partial_delta: torch.Tensor | None = None

        aux = self._blank_aux() if return_aux else None

        for layer_idx, block in enumerate(self.blocks):
            h, stats = block.pre_attn_mixer(
                self._build_sources(block_reps, block_input, partial_delta),
            ...
            partial_delta = attn_out if partial_delta is None else partial_delta + attn_out

            h, stats = block.pre_mlp_mixer(
                self._build_sources(block_reps, block_input, partial_delta),
                return_stats=return_aux,
            )
            if return_aux and aux is not None and stats is not None:
            ...
            partial_delta = partial_delta + mlp_out

            if (layer_idx + 1) % self.block_size_layers == 0:
                block_input = block_input + partial_delta
                block_reps.append(block_input)
                partial_delta = None

        h, out_stats = self.output_mixer(
            self._build_sources(block_reps, block_input, partial_delta),
            return_stats=return_aux,
        )
        if return_aux and aux is not None and out_stats is not None:
